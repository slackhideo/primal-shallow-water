...
class CPlayerInput 
{
	...
	event OnPNDrinkShallowWater( action : SInputAction ) {
		if (IsPressed(action))
		{
			pressPSWDrinkShallowWaterTime = theGame.GetEngineTimeAsSeconds();
			thePlayer.AddTimer('PSWBottleWaterTimer', KEY_HOLD_WINDOW, false);
		}
		if (IsReleased(action))
		{
			thePlayer.RemoveTimer('PSWBottleWaterTimer');
			if (theGame.GetEngineTimeAsSeconds() - pressPSWDrinkShallowWaterTime < KEY_HOLD_WINDOW)
			{
				PNDrinkShallowWater();
			}
		}
