...
import class CInventoryComponent extends CComponent
{
	...
	//modSigns
	public function AddMasterArmorerItems()
	{
	...
	public final function RemoveAllOilAbilitiesFromItem( id : SItemUniqueId )
	{
	...
	//modSigns
	public final function GetOilProtectionAgainstMonster( weaponId : SItemUniqueId, monsterCategory : EMonsterCategory ) : float
	{
		var i : int;
		var oils : array< W3Effect_Oil >;
		var bonus : float;
	...
	//modSigns
	public final function GetOilPoisonChanceAgainstMonster( weaponId : SItemUniqueId, monsterCategory : EMonsterCategory ) : float
	{
		var i : int;
		var oils : array< W3Effect_Oil >;
		var bonus : float;
	...
	public final function GetParamsForRunewordTooltip(runewordName : name, out i : array<int>, out f : array<float>, out s : array<string>)
	{
		...
		switch(runewordName)
		{
			case 'Glyphword 2': //modSigns: new mechanic
			...
			//modSigns: Runeword 1 description
			case 'Runeword 1' :
			...
			case 'Runeword 12' :
				...
				//f.PushBack(max.valueAdditive); //modSigns
				break;
			default:
				break;
		}
	...
	//modSigns: redone to work by item name, now also supports more than one contained ability
	public final function GetPotionAttributesForTooltip(itemID : SItemUniqueId, out tips : array<SAttributeTooltip>)
	{
		GetPotionAttributesByName(GetItemName(itemID), tips, itemID);
	}
	
	public final function GetPotionAttributesByName(itemName : name, out tips : array<SAttributeTooltip>, optional potionId : SItemUniqueId)
	{
		...
		if(dm.ItemHasTag(itemName, 'Mutagen')) //modSigns: mutagen potions (decoctions) are handled differently now
			return;
				...
				if(buffType != EET_Undefined)
				{
					...
					for(k = 0; k < attrs.Size(); k += 1)
					{
						...
						if(buffType == EET_MariborForest && attrs[k] == 'focus_gain')
						{
							...
							newAttr.percentageValue = true; //modSigns: it's percentage!
						}
						else if(buffType == EET_GoldenOriole && attrs[k] == 'poison_resistance_perc') //modSigns: why, game, why?
						{
							newAttr.value = val.valueAdditive;
							newAttr.percentageValue = true;
						}
						else if(buffType == EET_Swallow && attrs[k] == 'bleeding_resistance_perc') //modSigns: why, game, why?
						{
							newAttr.value = val.valueAdditive;
							newAttr.percentageValue = true;
						}
						//why on Earth this shit exists? don't you have tooltip_settings.csv for this kinda stuff? hacky hacks, dammit...
		...
		{
			/* Toxicity */
			newAttr.originName = 'toxicity';
			newAttr.attributeName = GetAttributeNameLocStr('toxicity', false);
			newAttr.value = PNGetShallowTox();
			newAttr.percentageValue = false;
			if (newAttr.value != 0.0) tips.Insert(0, newAttr);
	...
	//modSigns
	public final function FillInMutagenDescription(potionId : SItemUniqueId, out uniqueDescription : string)
	{
		var i			: int;
		var buffType	: EEffectType;
		var abilityName	: name;
	...
	//modSigns
	public final function FillInMutagenDescriptionByName(potionName : name, out uniqueDescription : string)
	{
		var i, j		: int;
		var buffType	: EEffectType;
		var abilityName	: name;
	...
	//modSigns
	private final function FillInMutagenDescriptionFromBuffType(buffType : EEffectType, locKey : string, out uniqueDescription : string)
	{
		var min, max	: SAbilityAttributeValue;
		...
		uniqueDescription += "<br><br><font color=\"#FBEDCF\">" + GetLocStringByKeyExt( "gm_toxicity_offset_description" ) + "</font>";
	}
	
	//modSigns
	public function GetBombAttributesForTooltip(item : SItemUniqueId, out itemStats : array<SAttributeTooltip>)
	{
		GetBombAttributesForTooltipByName(GetItemName(item), itemStats);
	}
	
	//modSigns
	public function GetBombAttributesForTooltipByName(itemName : name, out itemStats : array<SAttributeTooltip>)
	{
		var dm							: CDefinitionsManagerAccessor = theGame.GetDefinitionsManager();
		var abs1, abs					: array<name>;
		var weights						: array<float>;
	...
	//modSigns
	private function FindAttrInTooltipSettings(attributeName : name, out attributeColor : string, out isPercentageValue : string) : bool
	{
	...
	//modSigns
	public function GetEnhancedBuffInfo(item : SItemUniqueId) : string
	{
		var dm							: CDefinitionsManagerAccessor = theGame.GetDefinitionsManager();
		var min, max					: SAbilityAttributeValue;
		var attrs						: array<name>;
	...
	public function GetItemBaseStats(itemId : SItemUniqueId, out itemStats : array<SAttributeTooltip>)
	{
		...
		//modSigns: additional bomb damages from skills
	...
	private function GetItemTooltipAttributes(itemId : SItemUniqueId, attributes : array<name>, out itemStats : array<SAttributeTooltip>):void
	{
		...
		for(i=0; i<settingsSize; i+=1)
		{
			...
			ExcludeEnhancements(itemId, attributeName, attributeVal); //modSigns
			stat.attributeColor = attributeColor;
			stat.percentageValue = isPercentageValue;			
			...
			if (stat.value != 0)
			{
				...
				//modSigns: replace effect duration with cloud duration
	...
	//modSigns
	private function ExcludeEnhancements(itemId : SItemUniqueId, attributeName : name, out attributeVal : SAbilityAttributeValue)
	{
	...
	public function GetItemStatsFromName(itemName : name, out itemStats : array<SAttributeTooltip>)
	{
		...
		for(i=0; i<settingsSize; i+=1)
		{
			...
			{
				//statLabel = GetAttributeNameLocStr('toxicity', false); //modSigns
	...
	public final function IsItemSetItem(item : SItemUniqueId) : bool
	{
		return
			...
			//modSigns
			ItemHasTag(item, theGame.params.ITEM_SET_TAG_KAER_MORHEN) ||
	...
	public function GetArmorType(item : SItemUniqueId, optional ignoreGlyphwords : bool) : EArmorType //modSigns
	{
		...
		//if( !ignoreGlyphwords ) //modSigns: reworked
		//{
	...
	//modSigns
	public function GetArmorTypeByName(item : name) : EArmorType
	{
	...
	function GenerateItemLevel( item : SItemUniqueId, rewardItem : bool ) : int
	{
		...
		else if ( !ItemHasTag( item, 'AutogenForceLevel') )
		{
			...
			switch( quality )
			{
			case 4: //relic and magic: -1...+1
			case 3:
				lvl = playerLevel + (int)(RandF()*2.9999) - 1;
				break;
			case 2: //master and common: -1...0
			case 1:
	...
	public function GetItemQualityFromName( itemName : name, out min : int, out max : int)
	{
		...
		min = Max(1, min); //modSigns
		max = Max(min, max); //modSigns
	}
	...
	//modSigns: set type by armor type, affected by glyphwords
	public function GetSetArmorType() : EArmorType
	{
	...
	//modSigns: if BaW is not installed, all witcher sets get generic armor bonuses
	public function IsSetArmorType(item : SItemUniqueId) : bool
	{
		return IsItemAnyArmor(item) && !ItemHasTag(item, 'gm_NoSetBonuses') &&
				(!theGame.GetDLCManager().IsEP2Enabled() || !ItemHasTag(item, 'SetBonusPiece'));
	}
	
	//modSigns: if BaW is not installed, all witcher sets get generic armor bonuses
	public function IsSetArmorTypeByName(itemName : name) : bool
	{
		var dm : CDefinitionsManagerAccessor;
		
		dm = theGame.GetDefinitionsManager();
		return dm.IsItemAnyArmor(itemName) && !dm.ItemHasTag(itemName, 'gm_NoSetBonuses') &&
				(!theGame.GetDLCManager().IsEP2Enabled() || !dm.ItemHasTag(itemName, 'SetBonusPiece'));
	}
	
	//modSigns: tooltip description
	public function GetSetArmorItemDescription(item : SItemUniqueId, out counterText : string, out descrText : string) : int
	{
	...
	//modSigns: crafting description
	public function GetSetArmorItemDescriptionByName(item : name, out descrText : string)
	{
		var armorType : EArmorType;
	...
	//modSigns: generic set descriptions
	public function GetSetArmorItemDescriptionByType(armorType : EArmorType, out descrText : string)
	{
		var paramsInt : array<int>;
	...
	//modSigns: used when calculating stamina penalty
	...
	//modSigns: used for generic armor sets
	public function CountSetArmorPieces(out pieces : array<int>)
	{
		var item : SItemUniqueId;
		var armors : array<SItemUniqueId>;
		var i : int;
